...
const Project = () => {
    ...
    const filteredProjects = (data?.tasks || []).filter((task) => {
        ...
        const matchManager = filters.projectManager
            ? task.projectManager?.toLowerCase().includes(filters.projectManager.toLowerCase())
            : true;
                ...
                {!data?.project &&
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', marginTop: 5 }}>
                        ...
                        <Stack direction={'row'} spacing={3}>
                            <MyButton name="Add Task" handler={() => setOpenCreate(true)} />
                            <CreateTaskModal
                                open={openCreate}
